...
import { getSongById, getLyricById } from "../../api/index";
...
const getSong = async () => {
  const res = await getSongById({ ids: id });
  songInfo.value = res.data.songs[0];
  // 获取-并调用_formatLyr方法, 处理歌词
  const lyrContent = await getLyricById({ id });
  const lyricStr = lyrContent.data.lrc.lyric;
  lyric.value = _formatLyr(lyricStr);
  // 初始化完毕先显示零秒歌词
...
const audioStart = () => {
  ...
  if (!playState.value) {
    // 如果状态为false
    audio.value.play(); // 调用audio标签的内置方法play可以继续播放声音
  } else {
    audio.value.pause(); // 暂停audio的播放
  }
  playState.value = !playState.value; // 点击设置对立状态
};
const showLyric = () => {
  // 监听播放audio进度, 切换歌词显示
  audio.value.addEventListener("timeupdate", () => {
    let curTime = Math.floor(audio.value.currentTime);
    // 避免空白出现
    if (lyric.value[curTime]) {
      curLyric.value = lyric.value[curTime];
      lastLy.value = curLyric.value;
    } else {
